...
class TestAsyncTokenManagement:
    ...
    @pytest.mark.asyncio
    async def test_refresh_token(self, async_async_sandbox):
        """Test refreshing JWT token."""
        # Access agent to ensure token is generated
        await async_sandbox.get_info()
        
        # Get initial token from cache
        initial_token_data = get_cached_token(async_sandbox.async_sandbox_id)
        ...
        new_token_data = get_cached_token(async_sandbox.async_sandbox_id)
